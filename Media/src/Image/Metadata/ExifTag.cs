...
/*
    Extracted from https://exiftool.org/TagNames/EXIF.html using the following script

    function makeArray() {
        var rows = temp1.children;
        var str = "";
        var dict = {};
        for (var i = 1; i < rows.length; i++) {
            var row = rows[i];
            var idElement = row.children[0];
            var nameElement = row.children[1];
            var names = nameElement.innerText.split("\n");
            if (nameElement.innerText.trim() !== "") {
                for (var j = 0; j < names.length; j++) {
                    var name = names[j];
                    if (dict[name] !== true) {
                        str += names[j].replace(" ", "").replace("-", "") + " = " + idElement.innerText + ",\n"; 
                        dict[name] = true;
                    }
                }
            }
    ...
    where temp1 is the <tbody> element of the table.
*/

/// <summary>
/// Basic extension methods for EXIF tags
/// </summary>
public static class ExifTagMethods {
    private static Regex toRealName = new Regex(@"(?<!^)(?=[A-Z])");
    public static string ToStringWithSpaces(this ExifTag tag)  => toRealName.Replace(tag.ToString(), " ");
}

/// <summary>
/// List of supported Exchangeable Image File Format tags
/// </summary>
